...
const STORAGE_KEY = "atc_final_clean_v3";
...
const DEFAULT_SITE = {
  ...
  products: [
    { title: "Complete Solar Setup", icon: "☀️", image: "https://images.unsplash.com/photo-1509391366360-2e959784a276?auto=format&fit=crop&w=1200&q=85", points: ["Rooftop solar planning", "Residential and commercial setup", "Panel, inverter and structure support", "After-sales service guidance"] },
    { title: "Complete Plumbing Solution", icon: "💧", image: "https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?auto=format&fit=crop&w=1200&q=85", points: ["Pipeline planning", "Project and home plumbing work", "Material and execution support", "Maintenance support"] },
    { title: "Complete Pumping Solution", icon: "🌊", image: "https://images.unsplash.com/photo-1581092921461-eab62e97a780?auto=format&fit=crop&w=1200&q=85", points: ["Borewell pumping setup", "Pump selection guidance", "Site-based installation support", "Reliable water flow solution"] }
  ],
  services: [
    { title: "Complete Solar Installation", icon: "⚡", image: "https://images.unsplash.com/photo-1613665813446-82a78c468a1d?auto=format&fit=crop&w=1200&q=85", points: ["Site survey and load planning", "Panel mounting and wiring", "Inverter setup and testing", "Service support after installation"] },
    { title: "Plumbing Work for Projects", icon: "🔧", image: "https://images.unsplash.com/photo-1607472586893-edb57bdc0e39?auto=format&fit=crop&w=1200&q=85", points: ["Project pipeline execution", "Bathroom and water-line work", "Professional fitting support", "On-site supervision"] },
    { title: "Civil Constructions", icon: "🏗️", image: "https://images.unsplash.com/photo-1581094794329-c8112a89af12?auto=format&fit=crop&w=1200&q=85", points: ["Residential and commercial work", "Foundation and structure support", "Project-based civil execution", "Quality-focused finishing work"] }
  ],
  benefits: [
...
function waUrl(site, message) {
  return "https://wa.me/91" + digits(site.company.whatsapp) + "?text=" + encodeURIComponent(message || "Hello, I want enquiry");
}

function callUrl(site) {
  return "tel:+91" + digits(site.company.phone);
}

function instaUrl(site) {
  return "https://instagram.com/" + String(site.company.instagram || "").replace("@", "");
}

function normalizeItem(item) {
  if (Array.isArray(item.points)) return item;
  const points = String(item.text || "").split("|").map((x) => x.trim()).filter(Boolean);
  return { ...item, points };
}
...
function hydrate(saved) {
  if (!saved) return clone(DEFAULT_SITE);
  return {
    ...clone(DEFAULT_SITE),
    ...saved,
    company: { ...clone(DEFAULT_SITE.company), ...(saved.company || {}) },
    hero: { ...clone(DEFAULT_SITE.hero), ...(saved.hero || {}) },
    ...
    benefits: saved.benefits || clone(DEFAULT_SITE.benefits),
    gallery: saved.gallery || clone(DEFAULT_SITE.gallery),
    faqs: saved.faqs || clone(DEFAULT_SITE.faqs)
  };
}
...
function loadLocalSite() {
  try {
    ...
    return raw ? hydrate(JSON.parse(raw)) : clone(DEFAULT_SITE);
  } catch {
    return clone(DEFAULT_SITE);
  }
}
...
async function fetchOnlineSite() {
  ...
  const site = hydrate(rows[0].data);
  localStorage.setItem(STORAGE_KEY, JSON.stringify(safeSite(site)));
  return site;
}

async function saveOnlineSite(site, accessToken) {
  ...
  const response = await fetch(`${SUPABASE_URL}/rest/v1/${SUPABASE_TABLE}`, {
    ...
    body: JSON.stringify({ id: SITE_ROW_ID, data: safe, updated_at: new Date().toISOString() })
  });
...
async function signInAdmin(email, password) {
  if (!SUPABASE_ENABLED) throw new Error("Supabase URL/key missing.");
  ...
  if (!response.ok || !data || !data.access_token) throw new Error("Wrong email/password or user not confirmed.");
...
async function signOutAdmin(accessToken) {
  try {
    if (SUPABASE_ENABLED && accessToken) {
      await fetch(`${SUPABASE_URL}/auth/v1/logout`, { method: "POST", headers: supabaseHeaders({}, accessToken) });
    }
  } catch {
    // ignore
  }
...
function Styles() {
  return (
    <style>{`
      ...
      @keyframes popupIn{0%{opacity:0;transform:translateY(35px) scale(.9)}100%{opacity:1;transform:translateY(0) scale(1)}}
      .float-soft{animation:floatSoft 4.5s ease-in-out infinite}
      .shine-text{background:linear-gradient(90deg,#fff,#67e8f9,#facc15,#fff);background-size:240%;-webkit-background-clip:text;color:transparent;animation:shineText 4s linear infinite}
      .card-hover{transition:.35s ease;position:relative;overflow:hidden}
      .card-hover:hover{transform:translateY(-8px);box-shadow:0 30px 70px rgba(15,23,42,.22)}
      .logo-wrap{position:relative;width:56px;height:56px;display:grid;place-items:center;border-radius:18px;background:linear-gradient(135deg,#0f172a,#0d47a1 50%,#22d3ee);box-shadow:0 10px 24px rgba(15,23,42,.22),0 0 18px rgba(34,211,238,.25)}
      .logo-wrap:before{content:"";position:absolute;inset:-2px;border-radius:20px;background:linear-gradient(135deg,rgba(34,211,238,.95),rgba(250,204,21,.65),rgba(13,71,161,.9));z-index:-1}
      .logo-core{position:relative;z-index:2;width:48px;height:48px;border-radius:16px;overflow:hidden;box-shadow:inset 0 0 14px rgba(255,255,255,.18)}
  ...
  if (type === "instagram") {
    return (
      <svg className={cls} viewBox="0 0 24 24" aria-hidden="true">
        ...
        <rect x="3" y="3" width="18" height="18" rx="5" fill="none" stroke="url(#igGradient)" strokeWidth="2.3" />
        <circle cx="12" cy="12" r="4" fill="none" stroke="url(#igGradient)" strokeWidth="2.3" />
        <circle cx="17.35" cy="6.65" r="1.25" fill="url(#igGradient)" />
      </svg>
    );
  }
  return <span>•</span>;
...
function SectionTitle({ small, title, text, dark = false }) {
  return (
    ...
    <header className={(scrolled ? "py-1 bg-white/95 shadow-2xl" : "py-3 bg-white/85 shadow-lg") + " fixed inset-x-0 top-0 z-50 transition-all duration-500 backdrop-blur-xl"}>
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 md:px-8">
        ...
        <nav className="hidden gap-6 lg:flex">
          {NAV.map((item) => <a key={item} href={"#" + item} className="text-sm font-black capitalize text-slate-700 hover:text-cyan-500">{item}</a>)}
        </nav>
        <div className="hidden gap-3 lg:flex">
          <a href={callUrl(site)} className="rounded-full border border-blue-950 px-5 py-2 font-black text-blue-950">Call</a>
          <a href={waUrl(site, "Hello, I want enquiry")} target="_blank" rel="noreferrer" className="rounded-full bg-cyan-400 px-5 py-2 font-black text-slate-950">WhatsApp</a>
        </div>
      ...
      {menu ? (
        <div className="border-t bg-white p-4 lg:hidden">
          {NAV.map((item) => <a key={item} onClick={() => setMenu(false)} href={"#" + item} className="block rounded-xl px-4 py-3 font-black capitalize text-slate-700">{item}</a>)}
        </div>
      ) : null}
    </header>
  );
    ...
    <section id="home" className="relative min-h-screen overflow-hidden bg-slate-950 pt-28 text-white">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-20 md:px-8 lg:grid-cols-2">
        <div>
          ...
          <h1 className="shine-text mt-7 text-5xl font-black md:text-7xl">{site.company.name}</h1>
          ...
          <div className="mt-8 flex flex-col gap-4 sm:flex-row">
            <a href={waUrl(site, "Hello, I want free consultation")} target="_blank" rel="noreferrer" className="rounded-full bg-cyan-400 px-8 py-4 text-center font-black text-slate-950 shadow-xl transition hover:-translate-y-1 hover:bg-cyan-300">Get Free Consultation</a>
            <a href={waUrl(site, "Hello, I want enquiry for " + site.company.tagline)} target="_blank" rel="noreferrer" className="rounded-full border border-white/30 bg-white/10 px-8 py-4 text-center font-black text-white backdrop-blur-xl transition hover:-translate-y-1 hover:bg-white hover:text-blue-950">WhatsApp Enquiry</a>
          </div>
        </div>
        <div className="float-soft rounded-[2rem] border border-white/10 bg-white/10 p-4 shadow-2xl backdrop-blur-xl">
    ...
    <section id="about" className="bg-white px-4 py-20 md:px-8">
      <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-2">
        ...
        <div>
          ...
          <a href={waUrl(site, "Hello, I want to know more about Ansh Trading Company")} target="_blank" rel="noreferrer" className="mt-7 inline-flex items-center gap-2 rounded-full bg-cyan-400 px-7 py-4 font-black text-slate-950 shadow-xl"><Icon type="whatsapp" /> Know More</a>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <div className="mx-auto mb-12 max-w-7xl">
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-3">
        {slides.map((item) => (
          <a key={item.title} href={waUrl(site, "I want enquiry for " + item.title)} target="_blank" rel="noreferrer" className="rounded-full bg-[#25D366] px-5 py-2 font-black text-white shadow"><span className="mr-2">{item.icon}</span>{item.title}</a>
        ))}
      </div>
    </div>
  );
...
function Cards({ site, id, small, title, items, white = false }) {
  return (
    <section id={id} className={(white ? "bg-white" : "bg-slate-50") + " px-4 py-20 md:px-8"}>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-3">
        {items.map((item, index) => {
          ...
          return (
            <div key={index} className="card-hover overflow-hidden rounded-[2rem] border border-slate-100 bg-white shadow-xl">
              {clean.image ? <img src={clean.image} alt={clean.title} className="h-56 w-full object-cover" /> : null}
              <div className="p-6">
                <div className="mb-4 grid h-16 w-16 place-items-center rounded-2xl bg-cyan-50 text-4xl shadow-inner">{clean.icon}</div>
                <h3 className="text-2xl font-black text-slate-950">{clean.title}</h3>
                {id === "products" || id === "services" ? (
                  <ul className="mt-5 space-y-3">
                    {clean.points.map((point, idx) => (
                      <li key={idx} className="flex gap-3 rounded-2xl bg-slate-50 p-3 text-sm font-bold leading-6 text-slate-700"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-cyan-400 text-xs font-black text-slate-950">✓</span><span>{point}</span></li>
                    ))}
                  </ul>
                ) : (
                  <p className="mt-3 leading-7 text-slate-600">{clean.text}</p>
                )}
                <a href={waUrl(site, "I want details for " + clean.title)} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 rounded-full bg-cyan-400 px-6 py-3 font-black text-slate-950"><Icon type="whatsapp" /> Enquire Now</a>
              </div>
            </div>
          );
        })}
    ...
    <section id="benefits" className="bg-blue-950 px-4 py-20 text-white md:px-8">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-3">
        {site.benefits.map((item, index) => (
          <div key={index} className="card-hover rounded-[2rem] border border-white/10 bg-white/10 p-8">
...
function Enquiry({ site }) {
  const options = [...site.products.map((x) => x.title), ...site.services.map((x) => x.title), "Dealership", "Associate"];
  const [form, setForm] = useState({ name: "", mobile: "", city: "", requirement: options[0] || "", message: "" });
  const [sent, setSent] = useState(false);
  const message = useMemo(() => `Hello ${site.company.name}, I want enquiry. Name: ${form.name || "-"}, Mobile: ${form.mobile || "-"}, City: ${form.city || "-"}, Requirement: ${form.requirement}, Message: ${form.message || "-"}`, [form, site.company.name]);
  ...
  async function sendToSheet() {
    ...
    try {
      await fetch(site.company.sheetWebhook, {
        ...
        body: JSON.stringify({ date: new Date().toLocaleString("en-IN"), ...form, source: site.company.name })
      });
    } catch {
      // ignore sheet error
    }
  ...
  function submitForm(event) {
    ...
    setSent(true);
    sendToSheet();
    window.open(waUrl(site, message), "_blank", "noopener,noreferrer");
    ...
    <section id="enquiry" className="relative overflow-hidden bg-slate-50 px-4 py-20 md:px-8">
      <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-2">
        <div>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {["Fast WhatsApp Reply", "Site Visit Support", "Dealer / Associate Query", "Project Work Enquiry"].map((item) => <a key={item} href={waUrl(site, "Hello, I want enquiry for " + item)} target="_blank" rel="noreferrer" className="rounded-2xl border border-cyan-100 bg-white p-4 font-black text-slate-800 shadow-lg">⚡ {item}</a>)}
          </div>
        </div>
        <form onSubmit={submitForm} className="card-hover rounded-[2rem] border border-white bg-white/90 p-6 shadow-2xl backdrop-blur-xl md:p-8">
          <div className="mb-5 flex items-center justify-between gap-4 rounded-2xl bg-gradient-to-r from-blue-950 to-cyan-500 p-5 text-white">
            <div><p className="text-sm font-black uppercase tracking-[.2em] text-cyan-100">WhatsApp Enquiry</p><h3 className="mt-1 text-2xl font-black">Submit Your Enquiry</h3></div>
            <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-[#25D366] text-white shadow-xl ring-4 ring-white/20"><Icon type="whatsapp" /></div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <select value={form.requirement} onChange={(e) => setForm({ ...form, requirement: e.target.value })} className="rounded-2xl border bg-slate-50 px-4 py-4 outline-none">{options.map((option) => <option key={option}>{option}</option>)}</select>
          </div>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <button className="inline-flex items-center justify-center gap-2 rounded-full bg-blue-950 px-7 py-4 font-black text-white"><Icon type="whatsapp" /> Submit & Open WhatsApp</button>
            <a href={waUrl(site, message)} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#25D366] px-7 py-4 text-center font-black text-white"><Icon type="whatsapp" /> Direct WhatsApp</a>
          </div>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {site.gallery.map((img, index) => (
            <a key={index} href={img || "#gallery"} target={img ? "_blank" : undefined} rel={img ? "noreferrer" : undefined} className="group relative overflow-hidden rounded-[1.8rem] bg-slate-100 shadow-xl ring-1 ring-slate-100 transition hover:-translate-y-2 hover:shadow-2xl">
              {img ? <img src={img} alt={"gallery " + (index + 1)} className="h-56 w-full object-cover transition duration-700 group-hover:scale-110" /> : <div className="grid h-56 place-items-center text-sm font-black text-slate-400">Add Photo URL</div>}
              <div className="absolute bottom-4 left-4 rounded-full bg-white/90 px-4 py-2 text-xs font-black text-slate-950 opacity-0 shadow-lg transition group-hover:opacity-100">View Photo {index + 1}</div>
            </a>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto max-w-4xl space-y-4">
        {site.faqs.map((item, index) => (
          <div key={index} className="rounded-2xl bg-white shadow">
            <button type="button" onClick={() => setActive(active === index ? -1 : index)} className="flex w-full justify-between p-5 text-left font-black">{item.q}<span>{active === index ? "−" : "+"}</span></button>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-2">
        <div>
          ...
          <div className="grid gap-4">
            <a href={callUrl(site)} className="flex items-center gap-3 rounded-2xl bg-white/10 p-5 font-black"><Icon type="phone" /> +91 {site.company.phone}</a>
            <a href={waUrl(site, "Hello, I want to connect")} target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-2xl bg-white/10 p-5 font-black text-[#25D366]"><Icon type="whatsapp" /> WhatsApp</a>
            <a href={"mailto:" + site.company.email} className="flex items-center gap-3 rounded-2xl bg-white/10 p-5 font-black"><Icon type="mail" /> {site.company.email}</a>
            <a href={instaUrl(site)} target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-2xl bg-white/10 p-5 font-black"><Icon type="instagram" /> @{site.company.instagram}</a>
        ...
        <div className="overflow-hidden rounded-[2rem] bg-white/10 p-3 shadow-2xl">
          ...
          <div className="p-4 text-center"><p className="font-black">📍 {site.company.location}</p></div>
        </div>
      </div>
    </section>
  );
...
function AdminPanel({ site, setSite, close, authSession, onLogout }) {
  ...
  async function save() {
    try {
      const result = await saveOnlineSite(site, authSession?.access_token);
      setSaveMsg(result.mode === "supabase" ? "Saved permanently online. Changes will show for everyone." : "Saved in this browser only. Add Supabase URL/key for permanent save.");
    } catch (err) {
      setSaveMsg(err.message || "Permanent save failed. Check Supabase policies.");
    }
  }
    ...
    return (
      <div className="grid gap-4">
        {site[section].map((item, index) => {
          const clean = normalizeItem(item);
          const pointsText = clean.points.join(" | ");
          return (
    ...
    <div className="fixed inset-0 z-[100] bg-black/70 p-4">
      <div className="mx-auto flex h-full max-w-6xl flex-col rounded-[2rem] bg-white shadow-2xl">
        <div className="flex justify-between border-b p-4">
          <div><h2 className="text-2xl font-black">Admin Panel</h2><p className="text-xs font-bold text-slate-500">Website content management</p></div>
          <div className="flex gap-2"><button type="button" onClick={onLogout} className="rounded-xl bg-red-500 px-4 py-2 font-black text-white">Logout</button><button type="button" onClick={close} className="rounded-xl bg-slate-900 px-4 py-2 font-black text-white">Close</button></div>
        </div>
            ...
            <div className="grid gap-4">
              ...
              {site.gallery.map((img, index) => (
                <div key={index} className="grid gap-3 rounded-2xl bg-slate-50 p-4 md:grid-cols-[180px_1fr]">
                  <div>{img ? <img src={img} alt={"gallery " + index} className="h-28 w-full rounded-2xl object-cover" /> : <div className="grid h-28 place-items-center rounded-2xl bg-white text-sm font-black text-slate-400">No Image</div>}</div>
                  <div>{field("image url " + (index + 1), img, (v) => { const next = clone(site); next.gallery[index] = v; setSite(next); })}<button type="button" onClick={() => { const next = clone(site); next.gallery.splice(index, 1); setSite(next); }} className="mt-3 rounded-xl bg-red-500 px-4 py-2 font-black text-white">Remove</button></div>
                </div>
              ))}
              <button type="button" onClick={() => { const next = clone(site); next.gallery.push(""); setSite(next); }} className="rounded-full bg-cyan-400 px-5 py-3 font-black">Add Image URL</button>
            </div>
          ) : null}
          {tab === "faqs" ? (
            <div className="grid gap-4">
              {site.faqs.map((item, index) => (
                <div key={index} className="grid gap-3 rounded-2xl bg-slate-50 p-4">
                  {field("question", item.q, (v) => { const next = clone(site); next.faqs[index].q = v; setSite(next); })}
                  {field("answer", item.a, (v) => { const next = clone(site); next.faqs[index].a = v; setSite(next); })}
                  <button type="button" onClick={() => { const next = clone(site); next.faqs.splice(index, 1); setSite(next); }} className="rounded-xl bg-red-500 px-4 py-2 font-black text-white">Remove FAQ</button>
                </div>
              ))}
              <button type="button" onClick={() => { const next = clone(site); next.faqs.push({ q: "New question", a: "New answer" }); setSite(next); }} className="rounded-full bg-cyan-400 px-5 py-3 font-black">Add FAQ</button>
            </div>
          ) : null}
        </div>
        <div className="border-t p-4"><p className="mb-2 text-sm font-black text-cyan-700">{saveMsg}</p><div className="flex justify-end gap-3"><button type="button" onClick={() => { setSite(clone(DEFAULT_SITE)); setSaveMsg("Reset done. Click Save to store reset data."); }} className="rounded-full bg-red-500 px-5 py-3 font-black text-white">Reset</button><button type="button" onClick={save} className="rounded-full bg-blue-950 px-5 py-3 font-black text-white">Save</button></div></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/80 p-4 backdrop-blur-md">
      <div className="relative max-w-lg overflow-hidden rounded-[2.4rem] border border-white/20 bg-white shadow-2xl" style={{ animation: "popupIn .55s cubic-bezier(.2,.9,.2,1) both" }}>
        ...
        <div className="relative bg-gradient-to-br from-blue-950 via-slate-950 to-cyan-700 p-8 text-white"><p className="inline-flex rounded-full border border-cyan-200/30 bg-white/10 px-4 py-2 text-xs font-black uppercase tracking-[.2em] text-cyan-100 backdrop-blur-xl">Consultation</p><h3 className="mt-5 text-3xl font-black leading-tight md:text-4xl">Need Solar, Plumbing or Pumping Solution?</h3><p className="mt-3 leading-7 text-slate-200">Send your enquiry and connect with our team.</p></div>
        <div className="p-7"><a href={waUrl(site, "Hello, I want consultation from Ansh Trading Company. Please share details.")} target="_blank" rel="noreferrer" className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#25D366] px-6 py-4 text-center font-black text-white"><Icon type="whatsapp" /> WhatsApp Enquiry Now</a><button type="button" onClick={() => setShow(false)} className="mt-3 w-full rounded-full border border-slate-200 px-6 py-3 font-black text-slate-700">Close</button></div>
      </div>
    </div>
  );
}
...
function Floating({ site }) {
  const items = [
    { label: "WhatsApp", type: "whatsapp", href: waUrl(site, "Hello, I want enquiry"), bg: "bg-[#25D366] text-white" },
    { label: "Call", type: "phone", href: callUrl(site), bg: "bg-cyan-400 text-slate-950" },
    { label: "Instagram", type: "instagram", href: instaUrl(site), bg: "bg-white text-slate-950" },
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[70] grid gap-3">
      {items.map((item) => <a key={item.label} href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel={item.href.startsWith("http") ? "noreferrer" : undefined} title={item.label} className={item.bg + " grid h-14 w-14 place-items-center rounded-full shadow-2xl transition hover:-translate-y-1 hover:scale-110"}><Icon type={item.type} /></a>)}
    </div>
  );
}

function App() {
  const [site, setSite] = useState(() => loadLocalSite());
  ...
  useEffect(() => {
    let active = true;
    fetchOnlineSite().then((onlineSite) => {
  ...
  useEffect(() => {
    const sync = () => setAdmin(window.location.hash === "#admin");
    sync();
  ...
  return (
    <div className="min-h-screen bg-white text-slate-950">
      <Styles />
      {loading ? <div className="fixed inset-0 z-[200] grid place-items-center bg-slate-950 text-white"><div className="text-center"><div className="mx-auto mb-4 h-16 w-16 animate-spin rounded-full border-4 border-cyan-400 border-t-transparent" /><div className="text-2xl font-black">Loading...</div></div></div> : null}
      <Header site={site} />
      <Hero site={site} />
      <About site={site} />
      <Cards site={site} id="products" small="Products" title="Our Product Solutions" items={site.products} />
      <Cards site={site} id="services" small="Services" title="Professional Services" items={site.services} white />
      <Benefits site={site} />
      <Enquiry site={site} />
      <Gallery site={site} />
      <FAQ site={site} />
